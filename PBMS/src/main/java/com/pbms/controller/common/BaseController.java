...
//			ExcelUtils.isFilePathExist(fileAbsoluteName);
	 ...
	 * @throws Exception
	 */
	public String receiveImageUploadFile(HttpServletRequest request,
			CommonsMultipartFile receiveFile) throws Exception {
		String fileAbsolutePath = "isNotFile";
		if (!receiveFile.isEmpty()) {
			String path = request.getSession().getServletContext()
					.getRealPath("/uploadFiles/"); // 获取本地存储路径
			// 常量参数启动,用于初始化参数
//			PropertiesUtil.PropertiesUtilInit("config.properties");
//			String path =PropertiesUtil.get("uploadPath");
...
//				throw new PanasignUploadImageException("msg:0000001", "图片不能大于500K");
				...
				// 新建一个文件
//				String fileAbsoluteName = path + "/" + ControlDataUtils.getSystemTimeToString()
//						+ fileType;
//				ExcelUtils.isFilePathExist(fileAbsoluteName);
//				File file = new File(fileAbsoluteName);
				// 将上传的文件写入新建的文件中
